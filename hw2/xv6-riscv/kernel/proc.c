...
myproc(void)
{
  ...
  struct cpu *c = mycpu();
  if(c->kthread != 0){
    struct proc *p = c->kthread->myprocess;
    pop_off();
    return p;
  }
  pop_off();
...
{
  struct proc *p = myproc();
  struct kthread* mykth = mykthread();
  for(struct kthread* kt = p->kthread; kt<&p->kthread[NKT];kt++){
    if(kt!=mykth){
      acquire(&kt->lock);
      if(kt->state != KTUNUSED && kt->state!=KTZOMBIE){
        release(&kt->lock);
        kthread_kill(kt->thid);
        kthread_join(kt->thid,&status);
      }
        release(&kt->lock);
    }
  }
...
wait(uint64 addr)
{
  ...
  for(;;){
    ...
    if(!havekids || killed(p) || (mykthread()->killed == 1)){
      release(&wait_lock);
      return -1;
    }
...
sched(void)
{
  ...
  //printf("in sched after the swtch\n");
  mycpu()->intena = intena;
}
